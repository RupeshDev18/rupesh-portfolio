      ...
      <div className="max-w-6xl mx-auto">
        ...
        <motion.div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8" initial="hidden" whileInView="visible" viewport={{ once: true }}>
          {blogPosts.map((post, idx) => (
            <motion.article key={idx} className="bg-white dark:bg-slate-900/60 dark:backdrop-blur-md rounded-lg overflow-hidden border border-gray-150 dark:border-teal-500/20 dark:hover:border-cyan-400/40 hover:shadow-lg dark:hover:shadow-[0_0_25px_rgba(6,182,212,0.15)] flex flex-col transition-all duration-300" whileHover={{ y: -5 }}>
              ...
              <div className="p-6 flex-1 flex flex-col">
                ...
                <Link to={`/blog/${post.id}`} className="hover:text-teal-500 transition-colors">
                  <h3 className="text-lg font-bold mb-3 text-gray-900 dark:text-white line-clamp-2">{post.title}</h3>
                </Link>
                <p className="text-gray-600 dark:text-gray-400 text-sm mb-4 flex-1">{post.excerpt}</p>

                <div className="mb-4">
                  <Link to={`/blog/${post.id}`} className="inline-flex items-center gap-1 text-sm font-semibold text-teal-600 dark:text-cyan-400 hover:text-blue-500 transition-colors">
                    Read Article <FiArrowRight />
                  </Link>
                </div>

                <div className="flex items-center justify-between text-xs text-gray-500 dark:text-gray-400 pt-4 border-t border-gray-200 dark:border-teal-950/50">
